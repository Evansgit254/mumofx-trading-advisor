...
def test_fetch_data_success(mock_ticker):
    ...
    # Indicator check (optional, but good)
    from indicators.calculations import IndicatorCalculator
    res_with_ind = IndicatorCalculator.add_indicators(res, "m15")
    assert 'ema_20' in res_with_ind.columns
...
async def test_get_latest_data_async():
    ...
    async def mock_fetch(symbol, tf, period="5d"):
        index = pd.date_range(end=pd.Timestamp.now(tz="UTC"), periods=100, freq=tf)
        return pd.DataFrame({
...
async def test_get_latest_data_missing_timeframes():
    ...
    async def mock_fetch_partial(symbol, tf, period="5d"):
        ...
        if symbol == 'EURUSD=X' and tf == ENTRY_TF:
            return None
        index = pd.date_range(end=pd.Timestamp.now(tz="UTC"), periods=10, freq=tf)
        return pd.DataFrame({
