...
def test_calculate_score_sweeps():
    # M15 Sweep
    assert ScoringEngine.calculate_score({'sweep_type': 'M15_SWEEP'}) == 4.5 # 1.5 (h1 False) + 3.0
    # M5 Sweep
    assert ScoringEngine.calculate_score({'sweep_type': 'M5_SWEEP'}) == 3.5 # 1.5 + 2.0
...
def test_calculate_score_gold_protections():
    ...
    # 3.0 (base) + 1.0 (asian sweep) - 1.5 (penalty for low quality) - 3.0 (gold penalty) = -0.5
    # Actually logic for asian_sweep is:
    # if asian_sweep and asian_quality: +1.0
    # elif asian_sweep and not asian_quality: -1.5
    # AND if Gold and asian_sweep and not asian_quality: additional -3.0
    # Total: 3.0 - 1.5 - 3.0 = -1.5
    assert ScoringEngine.calculate_score(details) == -1.5

def test_calculate_score_asian_sweeps():
    # Good Asian Sweep
    details = {'asian_sweep': True, 'asian_quality': True}
    # 1.5 (base) + 1.0 (V6) + 0.5 (V8/V5) = 3.0
    assert ScoringEngine.calculate_score(details) == 3.0
...
def test_calculate_score_alpha_symbols():
    # JPY Bonus
    assert ScoringEngine.calculate_score({'symbol': 'USDJPY=X'}) == 2.5 # 1.5 + 1.0
    # NASDAQ Bonus
    assert ScoringEngine.calculate_score({'symbol': '^IXIC'}) == 2.5 # 1.5 + 1.0
